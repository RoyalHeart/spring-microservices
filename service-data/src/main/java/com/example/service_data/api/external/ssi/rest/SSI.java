...
public class SSI {
    ...
    private final String GET_HIST_URI = "/statistics/charts/history";
    private final String SSI_URL = "https://iboard-api.ssi.com.vn";

    public SsiPriceResponse getPrice(PriceRequest priceRequest) {
        RestClient restClient = RestClient.builder().baseUrl(SSI_URL).build();
        RequestHeadersSpec<?> request = restClient.get()
                .uri(GET_HIST_URI + "?resolution={resolution}&symbol={symbol}&from={from}&to={to}",
